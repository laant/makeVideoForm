...
- **씬 템플릿 5종** — `title` · `compare` · `list` · `demo` · `cta`
- **연출 라이브러리** — 트랜지션 20종 · 화면 효과 19종 · 효과음 17종 (전부 FFmpeg, 저작권 걱정 없음). 목록: [templates/scenes/README.md](templates/scenes/README.md)
- **자동 검수** — 해상도·FPS·코덱·길이·검은 화면·무음 구간·첫 소리/마지막 발화 잘림·자리표시자를 확인하고, 씬별 프레임을 한 장에 모은 이미지를 만듭니다.
- **업로드 전 점검** — `upload`·`publish` 가 자리표시자, 링크 안내↔실제 URL, URL 접속, 글자 수를 확인하고 실패하면 업로드를 막습니다. 사람이 볼 항목은 체크리스트로 출력합니다.
